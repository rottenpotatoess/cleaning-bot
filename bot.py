import os
import logging
import asyncio
from datetime import datetime
import pytz
import pandas as pd
from apscheduler.schedulers.asyncio import AsyncIOScheduler
from telegram import Bot
from telegram.error import TelegramError
from dotenv import load_dotenv

load_dotenv()

logging.basicConfig(
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    level=logging.INFO,
)
logger = logging.getLogger(__name__)

TELEGRAM_BOT_TOKEN = os.environ["TELEGRAM_BOT_TOKEN"]
TELEGRAM_GROUP_ID = os.environ["TELEGRAM_GROUP_ID"]
EXCEL_FILE = os.environ.get("EXCEL_FILE", "Coffee_Cleaning_Schedule_2026.xlsx")
CAMBODIA_TZ = pytz.timezone("Asia/Phnom_Penh")

MAX_RETRIES = 3
RETRY_DELAY_SECONDS = 30  # wait 30 seconds before retrying


def escape_md(text: str) -> str:
    special_chars = r"\_*[]()~`>#+-=|{}.!"
    for char in special_chars:
        text = text.replace(char, f"\\{char}")
    return text


def load_schedule() -> pd.DataFrame:
    df = pd.read_excel(EXCEL_FILE)
    df.columns = df.columns.str.strip()
    df["Date"] = pd.to_datetime(df["Date"], format="mixed")
    return df


def get_todays_shift(df: pd.DataFrame) -> dict | None:
    today = datetime.now(CAMBODIA_TZ).date()
    for _, row in df.iterrows():
        shift_date = row["Date"].date()
        if shift_date == today:
            return {
                "date": shift_date,
                "team": str(row["Team"]).strip(),
                "members": str(row["Assigned Members"]).strip(),
            }
    return None


def build_message(shift: dict) -> str:
    date_str = escape_md(shift["date"].strftime("%A, %B %d, %Y"))
    team = escape_md(shift["team"])
    members_list = [escape_md(m.strip()) for m in shift["members"].split(",")]
    members_formatted = "\n".join(f"  • {m}" for m in members_list)

    return (
        f"☕ *ដល់ម៉ោងលាងម៉ាស៊ីនកាហ្វេហើយបងៗ* ☕\n\n"
        f"☕ *Coffee Machine Cleaning Reminder* ☕\n\n"
        f"📅 *Date:* {date_str}\n"
        f"👥 *Team:* {team}\n\n"
        f"🧹 *អ្នកលាង:*\n{members_formatted}\n\n"
        f"🧹 *Assigned Members:*\n{members_formatted}\n\n"
        f"⏰ សូមជួយលាងម៉ាស៊ីនកាហ្វេផងណា\\! 🙏\n"
        f"⏰ Please remember to clean the coffee today\\. Thank you\\! 🙏"
    )


async def send_with_retry(bot: Bot, message: str) -> bool:
    """Try to send message up to MAX_RETRIES times. Returns True if successful."""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            await bot.send_message(
                chat_id=TELEGRAM_GROUP_ID,
                text=message,
                parse_mode="MarkdownV2",
            )
            logger.info(f"Message sent successfully on attempt {attempt}")
            return True

        except TelegramError as e:
            logger.error(f"Attempt {attempt}/{MAX_RETRIES} failed: {e}")
            if attempt < MAX_RETRIES:
                logger.info(f"Retrying in {RETRY_DELAY_SECONDS} seconds...")
                await asyncio.sleep(RETRY_DELAY_SECONDS)
            else:
                logger.error("All retry attempts failed. Message was not sent.")

    return False


async def send_reminder():
    logger.info("Checking schedule for today...")
    try:
        df = load_schedule()
        shift = get_todays_shift(df)

        if shift is None:
            logger.info("No cleaning shift scheduled for today.")
            return

        logger.info(f"Found shift: {shift}")
        message = build_message(shift)

        bot = Bot(token=TELEGRAM_BOT_TOKEN)
        success = await send_with_retry(bot, message)

        if success:
            logger.info(f"Reminder sent successfully to group {TELEGRAM_GROUP_ID}")
        else:
            logger.error(f"Failed to send reminder after {MAX_RETRIES} attempts.")

    except Exception as e:
        logger.error(f"Unexpected error: {e}", exc_info=True)


async def main():
    scheduler = AsyncIOScheduler(timezone=CAMBODIA_TZ)

    scheduler.add_job(
        send_reminder,
        trigger="cron",
        hour=16,
        minute=30,
        id="daily_reminder",
        name="Coffee Cleaning Reminder",
        replace_existing=True,
    )

    scheduler.start()
    logger.info("Bot started. Scheduler running — daily reminder at 4:30 PM (Cambodia time).")

    try:
        while True:
            await asyncio.sleep(60)
    except (KeyboardInterrupt, SystemExit):
        scheduler.shutdown()
        logger.info("Bot stopped.")


if __name__ == "__main__":
    asyncio.run(main())
